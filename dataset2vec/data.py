...
class Dataset2VecLoader:
    def __init__(
        self, data_path: Path, batch_size: int = 32, n_batches: int = 100
    ):
        self.data_path = data_path
        self.batch_size = batch_size
        self.n_batches = n_batches
        self.Xs = [
            pd.read_csv(path).iloc[:, :-1] for path in data_path.iterdir()
        ]
        ...
        self.ys = [
            torch.Tensor(pd.read_csv(path).iloc[:, [-1]].values)
    ...
    def __get_single_example(self):
        ...
        dataset_2_idx = (
            dataset_1_idx if np.random.uniform() < 0.5 else datasets_ids[1]
        )

        X1, y1 = self.Xs[dataset_1_idx], self.ys[dataset_1_idx]
        X2, y2 = self.Xs[dataset_2_idx], self.ys[dataset_2_idx]

        rows_idx_1, features_idx_1, targets_idx_1 = self.__sample_batch_idx(
            X1, y1
        )
        X1 = X1[rows_idx_1].T[features_idx_1].T
        y1 = y1[rows_idx_1].T[targets_idx_1].T

        rows_idx_2, features_idx_2, targets_idx_2 = self.__sample_batch_idx(
            X2, y2
        )
...
class Dataset2VecValidationLoader:
    def __init__(self, data_path: Path, batch_size: int = 32):
        loader = Dataset2VecLoader(
            data_path=data_path, batch_size=batch_size, n_batches=1
        )
